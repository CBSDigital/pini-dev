...
from . import sgc_container, sgc_utils, sgc_ety
...
class SGCProj(sgc_container.SGCContainer):
    ...
    def _read_assets(self, force=False):
        ...
        _data = self._read_data(sgc_ety.SGCAsset, force=force)
        _assets = []
        for _item in _data:
            _LOGGER.debug(' - ADD ITEM %s', _item)
            _asset = sgc_ety.SGCAsset(_item, proj=self)
            _assets.append(_asset)
        return _assets
    ...
    def _read_shots(self, progress=False, force=False):
        ...
        _data = self._read_data(
            entity_type='Shot',
            fields=sgc_ety.SGCShot.FIELDS,
            force=force)
        _LOGGER.debug(' - FOUND %d SHOTS ENTRIES', len(_data))
        _shots = []
        for _item in _data:
            _LOGGER.debug(' - ADD ITEM %s', _item)
            if not _item.get('sg_sequence'):
                continue
            _shot = sgc_ety.SGCShot(_item, job=self)
            _shots.append(_shot)
        return _shots
