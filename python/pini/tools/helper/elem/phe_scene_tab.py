...
from .phe_output_item import OutputItem
from .phe_scene_ref_item import SceneRefItem
...
class CLSceneTab(object):
    ...
    def _ctx_scene_refs_add_update_tag_opts(self, menu, refs):
        ...
        # Add tag opts
        _tags = sorted(
            {_out.tag for _out in _out.work_dir.outputs},
            key=pipe.tag_sort)
        for _tag in _tags:

            # Determine icon/enabled
            _latest = single(
                _out.work_dir.find_outputs(tag=_tag, ver_n='latest'))
            if _tag == _out.tag:
                _icon = CURRENT_ICON
                _enabled = False
            else:
